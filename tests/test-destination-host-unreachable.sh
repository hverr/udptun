...
! docker exec udptun2 ping -W 1 -c 3 192.168.111.50 2>&1 | tee /tmp/ping-$$.log
if ! grep "Destination Host Unreachable" /tmp/ping-$$.log; then
    echo "Error: expected destination host unreachable" >&2
    exit 1
fi
